...
function openDayModal(isoDate){ const item=DAY_DETAILS[isoDate]; if(!item) return;
  const title=document.getElementById("modalTitle"); const stats=document.getElementById("modalStats"); const body=document.getElementById("modalTableBody");
...
let tooltipEl=null;
function updateTooltipPosition(circle){ if(!tooltipEl) return; tooltipEl.style.display='block'; const circleRect=circle.getBoundingClientRect(); const px=circleRect.left+circleRect.width/2; const circleTop=circleRect.top; const offset=8; if(!tooltipEl){ tooltipEl=document.createElement('div'); tooltipEl.className='graph-tooltip'; tooltipEl.style.display='none'; document.body.appendChild(tooltipEl); } const tooltipHeight=tooltipEl.offsetHeight; const tooltipWidth=tooltipEl.offsetWidth; let tooltipTop=circleTop-tooltipHeight-offset; let tooltipLeft=px+12; tooltipEl.style.left=tooltipLeft+'px'; tooltipEl.style.top=tooltipTop+'px'; const tooltipRect=tooltipEl.getBoundingClientRect(); if(tooltipRect.right>window.innerWidth-10) tooltipEl.style.left=(px-tooltipWidth-12)+'px'; if(tooltipRect.left<10) tooltipEl.style.left='10px'; if(tooltipRect.top<10) tooltipEl.style.top=(circleRect.bottom+offset)+'px'; const finalRect=tooltipEl.getBoundingClientRect(); if(finalRect.bottom>window.innerHeight-10) tooltipEl.style.top=(window.innerHeight-finalRect.height-10)+'px'; }
function initGraphTooltip(){ if(!tooltipEl){ tooltipEl=document.createElement('div'); tooltipEl.className='graph-tooltip'; tooltipEl.style.display='none'; document.body.appendChild(tooltipEl); } function tryInit(){ const circles=document.querySelectorAll('svg circle[data-tooltip-date]'); if(circles.length) attachTooltipListeners(circles); else setTimeout(tryInit,100); } setTimeout(tryInit,50); setTimeout(tryInit,200); setTimeout(tryInit,500); }
function attachTooltipListeners(circles){ circles.forEach(circle=>{ if(circle.hasAttribute('data-tooltip-attached')) return; circle.setAttribute('data-tooltip-attached','true'); circle.style.pointerEvents='auto'; circle.addEventListener('mouseenter',e=>{ e.stopPropagation(); const c=e.target; const date=c.getAttribute('data-tooltip-date'); const avg=c.getAttribute('data-tooltip-avg'); const median=c.getAttribute('data-tooltip-median'); const p25=c.getAttribute('data-tooltip-p25'); const p75=c.getAttribute('data-tooltip-p75'); const min=c.getAttribute('data-tooltip-min'); const max=c.getAttribute('data-tooltip-max'); const count=c.getAttribute('data-tooltip-count'); if(!date) return; if(!tooltipEl){ tooltipEl=document.createElement('div'); tooltipEl.className='graph-tooltip'; tooltipEl.style.display='none'; document.body.appendChild(tooltipEl); } tooltipEl.innerHTML=`<div class="tooltip-header">${escHtml(date)}</div><div class="tooltip-section"><div class="tooltip-row"><span class="tooltip-label">平均</span><span class="tooltip-value">${escHtml(avg)}</span></div><div class="tooltip-row"><span class="tooltip-label">中央値</span><span class="tooltip-value">${escHtml(median)}</span></div></div><div class="tooltip-section"><div class="tooltip-row"><span class="tooltip-label">下位25%点</span><span class="tooltip-value">${escHtml(p25)}</span></div><div class="tooltip-row"><span class="tooltip-label">上位25%点</span><span class="tooltip-value">${escHtml(p75)}</span></div></div><div class="tooltip-section"><div class="tooltip-row"><span class="tooltip-label">最小</span><span class="tooltip-value">${escHtml(min)}</span></div><div class="tooltip-row"><span class="tooltip-label">最大</span><span class="tooltip-value">${escHtml(max)}</span></div></div><div class="tooltip-row" style="margin-top:8px; padding-top:8px; border-top:1px solid #f3f4f6;"><span class="tooltip-label">件数</span><span class="tooltip-value">${escHtml(count)}</span></div>`; updateTooltipPosition(c); }); circle.addEventListener('mouseleave',e=>{ e.stopPropagation(); if(tooltipEl) tooltipEl.style.display='none'; }); circle.addEventListener('mousemove',e=>{ e.stopPropagation(); if(!tooltipEl||tooltipEl.style.display==='none') return; updateTooltipPosition(e.target); }); }); }
</script>"""
